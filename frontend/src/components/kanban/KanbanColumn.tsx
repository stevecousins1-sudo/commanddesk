...
export default function KanbanColumn({ id, label, tasks, onAddTask, onDeleteTask }: Props) {
  ...
  return (
    ...
    >
      ...
      <div ref={setNodeRef} className="flex-1 p-3 space-y-2 overflow-y-auto" style={{ minHeight: 80 }}>
        {tasks.map(task => (
          <KanbanCard key={task.id} task={task} onDelete={onDeleteTask ? () => onDeleteTask(task.id) : undefined} />
        ))}
        {tasks.length === 0 && (
          <div
